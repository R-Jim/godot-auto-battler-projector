...
func test_add_stat_modifier():
	equipment.add_stat_modifier("attack", PropertyProjector.Modifier.Op.ADD, 10.0, 5)
	assert_eq(equipment.modifiers.size(), 1)
	
	var mod = equipment.modifiers[0]
	assert_eq(mod.id, "_attack_0")
	assert_eq(mod.op, PropertyProjector.Modifier.Op.ADD)
...
func test_add_multiplicative_stat():
	...
	assert_eq(equipment.modifiers.size(), 1)
	var mod = equipment.modifiers[0]
	assert_eq(mod.op, PropertyProjector.Modifier.Op.MUL)
