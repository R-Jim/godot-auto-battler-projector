...
var modifiers: Array[PropertyProjector.Modifier] = []
...
func add_stat_modifier(stat: String, op: PropertyProjector.Modifier.Op, value: float, priority: int = 0) -> void:
	var mod_id = id + "_" + stat + "_" + str(modifiers.size())
	var mod = PropertyProjector.Modifier.new(
		mod_id,
		op,
		value,
		priority,
...
func add_additive_stat(stat: String, value: float, priority: int = 0) -> void:
	add_stat_modifier(stat, PropertyProjector.Modifier.Op.ADD, value, priority)

func add_multiplicative_stat(stat: String, value: float, priority: int = 0) -> void:
	add_stat_modifier(stat, PropertyProjector.Modifier.Op.MUL, value, priority)
...
func equip_to(unit: BattleUnit) -> bool:
	...
	for mod in modifiers:
		for stat in mod.applies_to:
			if unit.projectors.has(stat):
				unit.projectors[stat].add_modifier(mod)
...
func unequip_from(unit: BattleUnit) -> void:
	...
	for mod in modifiers:
		for stat in mod.applies_to:
			if unit.projectors.has(stat):
				unit.projectors[stat].remove_modifier(mod)
...
func get_stat_bonuses() -> Dictionary:
	...
	for mod in modifiers:
		for stat in mod.applies_to:
			...
			match mod.op:
				PropertyProjector.Modifier.Op.ADD:
...
func clone() -> Equipment:
	...
	for mod in modifiers:
		new_equipment.modifiers.append(PropertyProjector.Modifier.new(
			mod.id,
			mod.op,
			mod.value,
			mod.priority,
